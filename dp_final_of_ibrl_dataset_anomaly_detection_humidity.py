...
first_filtered_dates.drop(
    first_filtered_dates.loc["2004-03-10":"2004-03-10"].index, inplace=True
)
second_filtered_dates.drop(
    second_filtered_dates.loc["2004-03-10":"2004-03-10"].index, inplace=True
)
...
first_agg_5m = first_filtered_dates.groupby(
    pd.Grouper(freq="30Min")
).aggregate(np.mean)

first_agg_5m.drop(
    first_agg_5m.loc["2004-03-10":"2004-03-10"].index, inplace=True
)

print(first_agg_5m)

second_agg_5m = second_filtered_dates.groupby(
    pd.Grouper(freq="30Min")
).aggregate(np.mean)

second_agg_5m.drop(
    second_agg_5m.loc["2004-03-15":"2004-03-16"].index, inplace=True
)
...
pd.DataFrame(second_filtered_dates).to_csv(
    "second_filtered_dates.csv", index=True
)
